...
def making_fld_continuous(filename):
	...
	while i < line_in_fld:
		...
		try:
			if len(line_to_append) > 1 and line_to_append[5] == ' ' and line_to_append[6] == '-' and line_to_append[
				7] == ' ':
				continous_fld.pop()
...
def rm_cmt_line_sign_start_line(filename):
	...
	with open(filename, 'r') as reader:
		for line in reader:
			line_to_append = line
			try:
				if len(line_to_append) > 1 and line_to_append[
					6] == '*':  # detecting comment if comment line then not write to output file
					continue
			except Exception as e:
	...
	error_d.close()
	rm_empty_line(
		out_put_file_name)  # due to above operation some empty line will be created so we have to remove them
	error_file_handle.write('\n*******************************************************************')
	return out_put_file_name
...
def remove_initail_head_area(filename):
	...
	with open(filename, 'r') as reader:
		for line in reader:
			...
			m = re.search('.([0-9]{1}).*',
			              line_to_append)  # regular expression for checking line of type 05 ,10 like this
...
def resolve_loop(filename):
	...
	while line_no < len(before_loop_fld):
		if 'OCCURS' in before_loop_fld[line_no] or 'occurs' in before_loop_fld[line_no]:
			...
			if ('OCCURS' in before_loop_fld[line_no] or 'occurs' in before_loop_fld[line_no]) and (
					'pic' in before_loop_fld[line_no] or 'PIC' in before_loop_fld[line_no]):
			...
			try:
				...
				print(color.RED + color.BOLD + "Unable to Find time of repition for the line",
				      before_loop_fld[line_no] + color.END)
			try:
...
def index_initilization(attribute_file, size_file):
	...
	for i in range(0, len(attribute)):
		...
		if attribute[i] in p_q_r_s.keys():
			code_file_index_selection.write('try:' + '\n')
			p_q='   ' +attribute[i] + ' = ' + 'trailing_negtive('+attribute[i]+')'+'\n'
			code_file_index_selection.write(p_q)
	...
	for i in range(0, len(attribute)):
		...
		if i < len(attribute) - 1:
			if attribute[i] in decimal_point.keys() and decimal_point[attribute[i]] > 0:
				create_table_with_this_data.write(
					attribute[i] + '\t\t' + 'varchar' + '(' + str(int(size[i]) + 1) + ' char' + ')' + ',')
			else:
				create_table_with_this_data.write(
					attribute[i] + '\t\t' + 'varchar' + '(' + str(size[i]) + ' char' + ')' + ',')
		else:
			if attribute[i] in decimal_point.keys() and decimal_point[attribute[i]] > 0:
				try:
					create_table_with_this_data.write(
...
def code_write(file_name, attribute_clean):
	...
	code_file.write("\ndef trailing_negtive(data):")
	code_file.write("\n	data=data.lower()")
	code_file.write("\n	length=len(data)")
	code_file.write("\n	neg_val=data[length-1]")
	code_file.write("\n	list = ['p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y']")
	code_file.write("\n	if neg_val in list:")
	code_file.write("\n		last_index=ord(neg_val)-ord('p')")
	code_file.write("\n		data=data.replace(neg_val,str(last_index))")
	code_file.write("\n		data='-'+data")
	code_file.write("\n	return data")
...
def resolve_p_q_r_s(filename):
	...
	while i<len(fld_line):
		line=fld_line[i]
		pic_s=re.search('pic(\s)+s',line)
		if pic_s is not None :
			if i+1<len(fld_line):
				next_line=fld_line[i+1]
				sign=re.search('.*leading\s+.*',next_line)
				if sign is None:
					line=line.lower()
					split=line.split('pic')
					split=split[0]
					split=re.sub('[0-9][0-9]','',split)
					split=split.strip()
...
def do_we_need_p_q(filename):
	fld_line = ''
	with open(filename, 'r') as reader:
		for line in reader:
			fld_line = fld_line + line
	reader.close()

	number_of_sign_start = re.findall('.*leading\s+.*', fld_line, re.IGNORECASE)
	number_of_s_pic = re.findall('pic(\s)+s', fld_line, re.IGNORECASE)

	print("print sign start :" + str(len(number_of_sign_start)))
	print("print pic with s :" + str(len(number_of_s_pic)))
	if len(number_of_s_pic) == len(number_of_sign_start):
		print("No tralling sign exist in the datafile ")
	else:
		resolve_p_q_r_s(root)
...
print(
	color.BLUE + color.BOLD + "Database Creation Mode is " + color.RED + color.BOLD + DATABASE_CREATION_MODE + color.END)
